...
@Service
public class ActualRouteFactory {

    private final Converter converter;

    public ActualRouteFactory() {
    ...
    public TreeMap<String, Route> getRoutesFromExcelFile(String pathFile) {
        ...
        System.out.println(now1 + ":STARTING READING EXCEL FILE");
        ExcelReader excelReader = new ExcelReader();
        HashMap<String, String> cellsFromExcelFile = excelReader.getCellsFromExcelFile(pathFile);
        LocalTime now2 = LocalTime.now();

        System.out.println(now2 + ": COMPLETED READING EXCEL FILE");
        System.out.println("----------------------------");
        LocalTime now3 = LocalTime.now();
        System.out.println(now3 + ": STARTING CREATING ROUTES FROM EXCEL FILE DATA");
        TreeMap<String, Route> routes = convertExcelDataToRoutes(cellsFromExcelFile);
        LocalTime now4 = LocalTime.now();
        System.out.println(now4 + ":CREATING ROUTES FROM EXCEL FILE DATA COMPLETED");
        System.out.println("----------------------------");
        return routes;
    }
    ...
    private Exodus addRowElementsToExodus(Exodus exodus, HashMap<String, String> data, int rowIndex) {
        ...
        // System.out.println("TripVoucherNumber: " + tripVoucherNumber);
